...
class PCAPlot(Figure):
    def __init__(self, train_features: np.ndarray, test_features: np.ndarray) -> None:
        ...
        index_array = np.concatenate(
            [np.arange(len(self.train_pca_data)), np.arange(len(self.test_pca_data))]
        )

        self.labels = ["train data"] * len(self.train_pca_data) + ["test data"] * len(
            self.test_pca_data
        )
        ...
        self.orig_point_source = ColumnDataSource(
            data={"comp1": np.full(1, np.nan), "comp2": np.full(1, np.nan)}
        )
