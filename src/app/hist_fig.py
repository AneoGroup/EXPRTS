...
class HistPlot(Figure):
    ...
    def __init__(self, train_features: np.ndarray, test_features: np.ndarray) -> None:
        ...
        for i, feature in enumerate(self.FEATURE_NAMES):
            ...
            orig_pos_source = ColumnDataSource(
                dict(
                    top=hist,
                    bottom=[0 for _ in range(len(hist))],
                    left=edges[:-1],
    ...
    @staticmethod
    def _set_source_selected(
        features: np.ndarray, sources: List[ColumnDataSource]
    ) -> None:
    ...
    def update_source(self) -> None:
        ...
        self._set_source_selected(
            self.features[self.active_index], self.orig_pos_source
        )
