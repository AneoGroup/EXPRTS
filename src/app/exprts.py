...
from bokeh.models import Button, Slider, Div, Panel, Tabs, TextInput, Dropdown, Label
...
class EXPRTS:
    def __init__(self, config: dict) -> None:
        ...
        self.model.load_state_dict(
            torch.load(os.path.join(config["path"], "model.pth")),
        )
    ...
    def _initialize_figure(self) -> None:
        ...
        metrics_tab = Tabs(
            tabs=[Panel(child=self.metric_plot.fig, title="Horizon score")]
        )
        info_tab = Tabs(
            tabs=[
                Panel(child=self.metric_plot.text, title="Metric scores"),
                Panel(child=self.hist_plot.text, title="Feature values"),
            ]
        )

        # ORGANIZE FIGURE
        # title1 = row(Div(text=f"Dataset: {self.config['dataset']}",
        #                 style={"font-size": "150%"}))
        # title2 = row(Div(text=f"Model: {self.config['model_name']}",
        #                 style={"font-size": "150%"}))

        plot_row1 = row(pca_hist_tab, metrics_tab, info_tab)
        plot_row2 = row(self.ts_plot.fig, self.manipulation_column)
        fig = column(plot_row1, plot_row2, sizing_mode="scale_both")
        self.plot_row1 = plot_row1
        self.plot_row2 = plot_row2
        self.fig = fig
