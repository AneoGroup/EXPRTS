...
def get_ts(
    index: int, test_data: List[pd.Series], len_train_data: int, config: dict
    ...
    if index >= len_train_data:
        ...
        ts = pd.read_csv(
            os.path.join(config["datadir"], "training_data", f"batch{batch}.csv")
        )
...
def get_prediction_dataloader(
    ...
    dataloader = ValidationDataLoader(
        dataset, batch_size=batch_size, stack_fn=batchify, transform=transformation
    )
    return dataloader
