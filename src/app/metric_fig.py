...
class MetricPlot(Figure):
    def __init__(self, horizon_scores: np.ndarray, sp: int, metric: str) -> None:
        ...
        self.fig = figure(
            title=self.metric.replace("_", " "),
            x_axis_label="horizon",
            y_axis_label=self.metric.replace("_", " "),
        ...
        self.fig.line(
            ...
            "test",
            source=self.source,
            legend_label=f"median test score",
            color=Category10_10[1],
        )
        self.fig.varea(
            "x",
            "test_upper",
            "test_lower",
            legend_label="50% confidence interval",
            source=self.source,
            color=Category10_10[1],
            fill_alpha=0.5,
        )
    ...
    def _calc_metric_score(
        self, target: np.ndarray, forecast: np.ndarray, ts: np.ndarray
    ) -> np.ndarray:
    ...
    def update_source(self) -> None:
        if self.orig_ts is not None and np.isnan(self.orig_scores).sum() == len(
            self.orig_scores
        ):
            target = self.orig_ts.values[-len(self.orig_forecast) :]
            self.orig_scores = self._calc_metric_score(
                target,
                self.orig_forecast,
        ...
        if self.modified_ts is not None:
            ...
            self.modified_scores = self._calc_metric_score(
                target,
                self.modified_forecast,
