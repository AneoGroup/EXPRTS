...
def score_batch(
    y_test: np.ndarray, y_pred: np.ndarray, y_train: np.ndarray, sp: int
) -> List[np.ndarray]:
...
def score_model(
    ...
    y_train: np.ndarray,
    sp: int = 1,
    metrics: List[str] = ["MASE", "sMAPE", "MAPE", "seasonal_MASE", "MSE"],
) -> dict:
    string_to_funcs = {
        "MASE": horizon_mase,
        "sMAPE": horizon_smape,
...
def mean_mase(
    y_test: np.ndarray, y_pred: np.ndarray, y_train: np.ndarray, sp: int = 1
) -> np.float32:
...
def horizon_mase(
    ...
    mae_naive_tiled = np.tile(
        mae_naive, [1, y_pred.shape[-1]]
    ...
    mase[
        ~mask
    ] = np.nan  # replace positions where denominator was 0 with undefined values
