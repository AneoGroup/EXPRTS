...
def trend_linearity(trend_comp: pd.Series) -> float:
    ...
    return max(
        0, 1 - np.var(residuals) / max(np.finfo(np.float32).eps, np.var(trend_comp))
    )
...
def extract_trend(ts: pd.DataFrame, filter_size: int = 7) -> DecomposeResult:
    ...
    ]  # remove points without a value from the moving average
...
def decomps_and_features(
    data: List[pd.DataFrame], sp: int, dataset_size: int = None
