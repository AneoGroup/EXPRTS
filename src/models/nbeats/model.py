...
class NBeats(BaseModel):
    ...
    def __init__(
        self, blocks: nn.ModuleList, sp: int, path: str, device: torch.device
    ) -> None:
        super().__init__()
        self.blocks = blocks
        self.sp = sp
    ...
    def validate(
        self, batch: Dict[str, torch.Tensor], sp: int = 1
...
class NBeatsBlock(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [nn.Linear(in_features=context_length, out_features=layer_size)]
            + [
...
class TrendBasis(nn.Module):
    ...
    def __init__(
        self, degree_of_polynomial: int, context_length: int, prediction_length: int
    ) -> None:
        super().__init__()
        ...
        self.backcast_time = nn.Parameter(
            torch.tensor(
                np.concatenate(
                    [
                        np.power(
                            np.arange(context_length, dtype=np.float) / context_length,
                            i,
                        )[None, :]
        ...
        self.forecast_time = nn.Parameter(
            torch.tensor(
                np.concatenate(
                    [
                        np.power(
                            np.arange(prediction_length, dtype=np.float)
                            / prediction_length,
                            i,
                        )[None, :]
                        for i in range(self.polynomial_size)
...
class SeasonalityBasis(nn.Module):
    ...
    def __init__(
        self, harmonics: int, context_length: int, prediction_length: int
    ) -> None:
        super().__init__()
        self.frequency = np.append(
            np.zeros(1, dtype=np.float32),
            np.arange(harmonics, harmonics / 2 * prediction_length, dtype=np.float32)
            / harmonics,
        )[None, :]
        backcast_grid = (
            -2
            * np.pi
            * (np.arange(context_length, dtype=np.float32)[:, None] / prediction_length)
            * self.frequency
        )
        forecast_grid = (
            2
            * np.pi
            * (
                np.arange(prediction_length, dtype=np.float32)[:, None]
                / prediction_length
            )
            * self.frequency
        )
