...
class EncoderRNN(torch.nn.Module):
    ...
    def forward(
        ...
        init_hidden = (
            torch.zeros(
                self.nlayers * self.rnn_directions,
                x.size(0),
                self.hidden_size,
                device=self.device,
            ),
...
class DecoderRNN(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, z: torch.Tensor, h: Tuple[torch.Tensor, torch.Tensor]
    ) -> torch.Tensor:
        # x: [batch_size, context_length, input_size]
        ...
        outputs = torch.zeros(
            [x.size(0), self.prediction_length, 1], device=self.device
        )
        for i in range(self.prediction_length):
            decoder_input = torch.cat(
                [output, z[:, -self.prediction_length + i, :].unsqueeze(dim=1)], dim=-1
            )
...
class Seq2Seq(BaseModel):
    def __init__(
        ...
        self.encoder = EncoderRNN(
            device, input_size, hidden_size, nlayers, bidirectional
        )
        self.decoder = DecoderRNN(
            prediction_length, device, input_size, hidden_size, nlayers
        )
    ...
    def _extract_input_from_batch(
        ...
        repeated_cat = embedded_cat.repeat_interleave(
            x.size(1) + self.prediction_length, dim=1
        ).float()
