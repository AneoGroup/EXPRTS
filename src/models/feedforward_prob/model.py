...
class FeedForwardProb(BaseModel):
    ...
    def __init__(
        ...
        self.df_proj = nn.Sequential(
            self.__make_lin(hidden_dimensions[-1], 1), nn.Softplus()
        )
    ...
    def calculate_loss(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if len(scaled_y.size()) > 2:
            scaled_y = scaled_y.squeeze(dim=-1)
    ...
    def predict(self, batch: Dict[str, torch.Tensor]) -> np.ndarray:
        ...
        return np.concatenate(
            [mean[..., np.newaxis], lower[..., np.newaxis], upper[..., np.newaxis]],
            axis=-1,
        )
