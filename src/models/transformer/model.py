...
class Transformer(BaseModel):
    def __init__(
        ...
        encoder_layers = nn.TransformerEncoderLayer(
            d_model, nheads, dim_feedforward, dropout
        )
    ...
    def _extract_input_from_batch(
        ...
        past_observed = (
            batch["past_observed_values"].type(torch.float32).to(self.device)
        )
        ...
        repeated_static_cat = static_cat.repeat_interleave(
            self.context_length, dim=-1
        ).type(torch.long)
        ...
        encoder_x = torch.cat(
            [x, embedded_static_cat, past_time_feat, past_observed.unsqueeze(dim=-1)],
            dim=-1,
        )
        encoder_x = encoder_x.permute(1, 0, 2).type(torch.float32).to(self.device)

        decoder_x = torch.cat(
            [embedded_static_cat[:, : self.prediction_length, :], future_time_feat],
            dim=-1,
        )
    ...
    def validate(
        self, batch: Dict[str, torch.Tensor], sp: Optional[int] = None
