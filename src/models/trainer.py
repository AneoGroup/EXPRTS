...
class Trainer:
    def __init__(
        ...
        if self.data_source == "local":
            ...
            ) = self._get_local_dataloaders(dataset)
        else:
            (
                self.train_dataloader,
                self.validation_dataloader,
            ) = self._get_gluon_dataloaders(dataset)
        ...
        if self.use_val_data:
            ...
            for i in range(self.num_validation_windows):
                for ts in train_data:
                    ...
                    if len(
                        ts["target"]
                    ) <= self.context_length + self.prediction_length * (i + 2):
                        continue

                    val_ts = deepcopy(ts)
                    val_ts["target"] = val_ts["target"][
                        : -self.prediction_length * i if i > 0 else None
                    ]
            ...
            return self._get_train_dataloader(
                train_data
            ), self._get_validation_dataloader(validation_data)
        else:
    ...
    def save_batch(self, batch: dict, datadir: str) -> None:
        df = pd.DataFrame()
        for i, (forecast_start, context, target, id) in enumerate(
            zip(
                batch["forecast_start"],
                batch["past_target"],
                batch["future_target"],
            ...
            start_time = pd.date_range(
                start=forecast_start.to_timestamp(),
                freq=-forecast_start.freq,
                periods=len(context) + 1,
            )[-1]
            index = pd.date_range(
                start_time, freq=forecast_start.freq, periods=ts_length
            )
        ...
        for epoch_no in range(1, self.epochs + 1):
            ...
            for batch_no, batch in enumerate(self.train_dataloader, start=1):
                ...
                if torch.isnan(torch.sum(loss)):
                    logger.critical(
                        f"NaN loss value, epoch {epoch_no} batch {batch_no}"
                    )
                    raise ValueError(
                        f"NaN loss value, epoch {epoch_no} batch {batch_no}"
                    )
                ...
                if (
                    self.data_source == "gluon"
                    and self.batches_written < self.num_batches_to_write
                ):
            ...
            if self.use_val_data and epoch_no % self.eval_every == 0:
                ...
                logger.info(
                    f"Epoch {epoch_no}, time spent: {round(time.time() - epoch_start, 1)}, "
            ...
            if (
                ...
                and scheduler.num_bad_epochs == self.patience
            ):
                logger.info(
                    "Stopping training due to lack of improvement in training loss"
                )
                break
